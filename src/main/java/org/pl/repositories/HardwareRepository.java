...
public class HardwareRepository {
    private final ArrayList<Hardware> elements;

    public void add(Hardware element) throws RepositoryException {
        if (element == null) {
            throw new RepositoryException(RepositoryException.REPOSITORY_ADD_INVALID_EXCEPTION);
        }
        for (Hardware t : elements) {
            if (t == element) {
                t.setArchive(false);
                return;
            }
